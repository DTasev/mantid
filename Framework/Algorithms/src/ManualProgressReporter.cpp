...
namespace Algorithms {
    ...
    const std::string ManualProgressReporter::name() const { return "ManualProgressReporter"; }

    /// Algorithm's version for identification. @see Algorithm::version
    int ManualProgressReporter::version() const { return 1; }

    /// Algorithm's category for identification. @see Algorithm::category
    const std::string ManualProgressReporter::category() const { return "Utility\\Development"; }
    ...
    {
        declareProperty(
            std::make_unique<Kernel::PropertyWithValue<int>>("NumberOfProgressReports", 10),
            "The number of times the progress will be reported.");
        declareProperty(
            std::make_unique<Kernel::PropertyWithValue<bool>>("StartAnotherAlgorithm", false),
            "The algorithm will start a child algorithm on every iteration, before reporting progress.");
    }
